...
const fmtDate = (d) => {
  ...
  return { month: o.toLocaleString("default", { month: "short" }).toUpperCase(), day: o.getDate(), year: o.getFullYear(), full: o.toLocaleDateString("default", { weekday: "long", year: "numeric", month: "long", day: "numeric" }) };
};
...
const T = {
  cream: "#F5F0E8", paper: "#EDE8DC", ink: "#2C2016", inkLight: "#4A3828",
  amber: "#5B9BAD", amberLight: "#7EC8D8", amberPale: "#D8EEF3",
  red: "#8B2020", groove: "#C4B99A", grooveLight: "#D8D0BC", stamp: "#6B4C1E",
};

const S = {
  input: { width: "100%", background: T.cream, border: `1.5px solid ${T.groove}`, borderRadius: "4px", color: T.ink, padding: "10px 14px", fontSize: "14px", outline: "none", boxSizing: "border-box", fontFamily: "'Outfit', sans-serif", transition: "border-color 0.2s" },
  label: { color: T.stamp, fontSize: "10px", letterSpacing: "2.5px", textTransform: "uppercase", marginBottom: "6px", display: "block", fontFamily: "'Outfit', sans-serif" },
};
  ...
  <div style={{ display: "flex", gap: "2px" }}>
    {[1,2,3,4,5].map(n => (
      <span key={n} onClick={() => onChange && onChange(n)}
        style={{ fontSize: size === "lg" ? "24px" : "15px", cursor: onChange ? "pointer" : "default", color: n <= v ? T.amber : T.grooveLight, transition: "color .15s", userSelect: "none" }}>♪</span>
    ))}
  </div>
);

const Groove = () => (
  <div style={{ height: "16px", display: "flex", flexDirection: "column", justifyContent: "center", gap: "3px" }}>
    {[0.12, 0.2, 0.14, 0.1].map((op, i) => <div key={i} style={{ height: "1px", background: `rgba(100,76,40,${op})` }} />)}
  </div>
);

const GenreSticker = ({ genre }) => (
  <span style={{ background: T.amberPale, border: `1px solid ${T.amber}`, borderRadius: "2px", padding: "2px 7px", fontSize: "9px", color: T.stamp, fontFamily: "'Outfit', sans-serif", letterSpacing: "1.5px", textTransform: "uppercase", fontWeight: "700" }}>{genre}</span>
);
    ...
    <div style={{ display: "flex", gap: "8px", flexWrap: "wrap", marginTop: "10px" }}>
      {(media || []).map((item, i) => (
        <div key={i} style={{ position: "relative", width: 72, height: 72 }}>
          {item.type === "video" ? <video src={item.url} style={{ width: 72, height: 72, objectFit: "cover", borderRadius: "3px", border: `1px solid ${T.groove}` }} /> : <img src={item.url} alt="" style={{ width: 72, height: 72, objectFit: "cover", borderRadius: "3px", border: `1px solid ${T.groove}` }} />}
          {onRemove && <button onClick={() => onRemove(i)} style={{ position: "absolute", top: 2, right: 2, background: "rgba(26,16,8,.7)", border: "none", borderRadius: "50%", color: T.cream, width: 18, height: 18, cursor: "pointer", fontSize: "10px", lineHeight: "18px", padding: 0 }}>✕</button>}
        </div>
      ))}
      {onAdd && (
        <div onClick={() => ref.current.click()} style={{ width: 72, height: 72, border: `1.5px dashed ${T.groove}`, borderRadius: "3px", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", cursor: "pointer", color: T.groove, fontSize: "10px", gap: "3px", transition: "all .2s" }}
...
const ConcertModal = ({ concert, onClose, onSave }) => {
  ...
  const handleSave = () => { if (!f.artist.trim()) return; onSave({ ...f, setlist: setlistInput.split("\n").map(s => s.trim()).filter(Boolean) }); };
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(26,16,8,.72)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center", padding: "20px", backdropFilter: "blur(3px)" }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ background: T.cream, border: `2px solid ${T.inkLight}`, borderRadius: "3px", padding: "32px", width: "100%", maxWidth: "560px", boxShadow: `6px 6px 0 ${T.groove}`, maxHeight: "92vh", overflowY: "auto" }}>
        <div style={{ borderBottom: `2px solid ${T.ink}`, paddingBottom: "16px", marginBottom: "24px", display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          <div>
            <div style={{ color: T.amber, fontSize: "10px", letterSpacing: "3px", textTransform: "uppercase", fontFamily: "'Outfit', sans-serif" }}>♪ NEW ENTRY</div>
        ...
        <div style={{ display: "flex", gap: "10px", marginTop: "24px", borderTop: `1px solid ${T.groove}`, paddingTop: "20px" }}>
          ...
          <button onClick={handleSave} style={{ flex: 2, padding: "11px", background: T.ink, border: "none", borderRadius: "3px", color: T.cream, cursor: "pointer", fontSize: "13px", fontWeight: "700", fontFamily: "'Fraunces', serif", opacity: f.artist.trim() ? 1 : 0.4 }}><Sym>▶</Sym> {concert?.id ? "Save Changes" : "Press It"}</button>
        </div>
      </div>
    </div>
  );
...
const SleeveModal = ({ concert, onClose, isOwn, onEdit }) => {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(26,16,8,.75)", zIndex: 300, display: "flex", alignItems: "center", justifyContent: "center", padding: "20px", backdropFilter: "blur(4px)" }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ background: T.paper, border: `2px solid ${T.ink}`, borderRadius: "2px", width: "100%", maxWidth: "620px", maxHeight: "92vh", overflowY: "auto", boxShadow: `8px 8px 0 ${T.inkLight}` }}>
        <div style={{ background: T.ink, padding: "36px 40px", position: "relative", overflow: "hidden" }}>
          {[220,180,140,100,60].map((size, i) => (<div key={i} style={{ position: "absolute", right: -(size/2.5), top: -(size/2.5), width: size, height: size, borderRadius: "50%", border: `1px solid rgba(255,255,255,${0.03 + i * 0.015})`, pointerEvents: "none" }} />))}
          <div style={{ color: T.amberLight, fontSize: "10px", letterSpacing: "4px", textTransform: "uppercase", fontFamily: "'Outfit', sans-serif", marginBottom: "12px" }}>♪ RECORD · LIVE PRESSING · {month} {day}, {year}</div>
          <h1 style={{ color: T.cream, fontFamily: "'Fraunces', serif", fontSize: "clamp(26px,5vw,42px)", fontWeight: "900", lineHeight: 1, margin: "0 0 10px", letterSpacing: "-1px" }}>{concert.artist}</h1>
          <div style={{ color: T.amberLight, fontFamily: "'Outfit', sans-serif", fontSize: "14px", marginBottom: "4px" }}>{concert.venue}</div>
          <div style={{ color: T.groove, fontFamily: "'Outfit', sans-serif", fontSize: "12px", marginBottom: "18px" }}>{concert.city} · {full}</div>
          <div style={{ display: "flex", alignItems: "center", gap: "14px", flexWrap: "wrap" }}><NoteRating v={concert.rating} size="lg" />{concert.genre && <GenreSticker genre={concert.genre} />}</div>
        ...
        <div style={{ padding: "32px 40px" }}>
          {concert.review && (<div style={{ marginBottom: "28px" }}><div style={{ ...S.label, marginBottom: "12px" }}>♪ Review</div><blockquote style={{ color: T.inkLight, fontFamily: "'Fraunces', serif", fontSize: "16px", lineHeight: 1.75, fontStyle: "italic", borderLeft: `3px solid ${T.amber}`, paddingLeft: "18px", margin: 0 }}>"{concert.review}"</blockquote></div>)}
          <Groove />
          {concert.setlist?.length > 0 && (<div style={{ margin: "24px 0" }}><div style={{ ...S.label, marginBottom: "14px" }}>♪ Setlist</div>{concert.setlist.map((song, i) => (<div key={i} style={{ display: "flex", alignItems: "center", gap: "14px", padding: "9px 0", borderBottom: `1px solid ${T.grooveLight}` }}><span style={{ color: T.grooveLight, fontFamily: "'Outfit', sans-serif", fontSize: "10px", minWidth: "22px", textAlign: "right" }}>{String(i+1).padStart(2,"0")}</span><span style={{ color: T.amber, fontSize: "11px" }}>♪</span><span style={{ color: T.ink, fontFamily: "'Fraunces', serif", fontSize: "16px" }}>{song}</span></div>))}</div>)}
          {concert.media?.length > 0 && (<><Groove /><div style={{ margin: "24px 0" }}><div style={{ ...S.label, marginBottom: "12px" }}>♪ Photos & Videos</div><div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>{concert.media.map((item, i) => (item.type === "video" ? <video key={i} src={item.url} controls style={{ height: 120, borderRadius: "3px", border: `1px solid ${T.groove}` }} /> : <img key={i} src={item.url} alt="" style={{ height: 120, borderRadius: "3px", objectFit: "cover", border: `1px solid ${T.groove}`, cursor: "pointer" }} onClick={() => window.open(item.url, "_blank")} />))}</div></div></>)}
          <div style={{ display: "flex", gap: "10px", marginTop: "24px", borderTop: `1px solid ${T.groove}`, paddingTop: "20px" }}>
            ...
            {isOwn && <button onClick={() => { onClose(); onEdit(concert); }} style={{ flex: 1, padding: "11px", background: T.ink, border: "none", borderRadius: "3px", color: T.cream, cursor: "pointer", fontSize: "13px", fontFamily: "'Outfit', sans-serif" }}><Sym>▶</Sym> Edit This Record</button>}
          </div>
        </div>
      </div>
    </div>
...
const TicketStub = ({ concert, onEdit, onDelete, onOpen, showOwner, ownerName, ownerHandle, ownerAvatar, isEncore }) => {
  ...
  return (
    <div onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)} onClick={() => onOpen(concert)}
      style={{ display: "flex", cursor: "pointer", border: `1.5px solid ${isEncore ? T.amber : T.inkLight}`, borderRadius: "3px", overflow: "hidden", transition: "transform .15s, box-shadow .15s", transform: hov ? "translateY(-2px)" : "none", boxShadow: hov ? `4px 4px 0 ${T.amber}` : `3px 3px 0 ${T.groove}`, background: T.cream }}>
      <div style={{ background: T.ink, width: 68, minWidth: 68, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "14px 6px", position: "relative" }}>
        ...
        <div style={{ color: T.amberLight, fontSize: "9px", fontWeight: "700", letterSpacing: "2px", fontFamily: "'Outfit', sans-serif" }}>{month}</div>
        <div style={{ color: T.cream, fontSize: "28px", fontFamily: "'Fraunces', serif", fontWeight: "900", lineHeight: 1 }}>{day}</div>
        <div style={{ color: T.groove, fontSize: "9px", fontFamily: "'Outfit', sans-serif" }}>{year}</div>
        <div style={{ color: T.amberLight, fontSize: "16px", marginTop: "6px" }}>♪</div>
      </div>
      <div style={{ flex: 1, padding: "14px 18px", display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
        <div>
          {showOwner && <div style={{ color: T.stamp, fontSize: "10px", fontFamily: "'Outfit', sans-serif", marginBottom: "4px" }}>{ownerAvatar} {ownerName} <span style={{ color: T.grooveLight }}>{ownerHandle}</span></div>}
          {isEncore && <div style={{ color: T.amber, fontSize: "9px", fontFamily: "'Outfit', sans-serif", letterSpacing: "2px", textTransform: "uppercase", marginBottom: "2px" }}>★ Encore List</div>}
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: "10px", flexWrap: "wrap", gap: "6px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "10px" }}><NoteRating v={concert.rating} />{concert.genre && <GenreSticker genre={concert.genre} />}</div>
      ...
      {(onEdit || onDelete) && (
        <div onClick={e => e.stopPropagation()} style={{ background: T.paper, borderLeft: `1px dashed ${T.groove}`, width: 40, minWidth: 40, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: "14px", opacity: hov ? 1 : 0, transition: "opacity .2s" }}>
...
const EncoreListSection = ({ concerts, encoreList, onOpen, onToggleEncore, isOwn }) => {
  ...
  return (
    <div style={{ background: T.paper, border: `2px solid ${T.amber}`, borderRadius: "4px", padding: "24px", marginBottom: "28px" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "18px" }}>
        <div>
          <div style={{ color: T.amber, fontSize: "10px", letterSpacing: "3px", textTransform: "uppercase", fontFamily: "'Outfit', sans-serif", marginBottom: "4px" }}>★ All-Time</div>
      ...
      {encoreConcerts.length === 0 ? (
        <div style={{ textAlign: "center", padding: "24px", color: T.groove, fontFamily: "'Outfit', sans-serif", fontSize: "13px", fontStyle: "italic", border: `1px dashed ${T.groove}`, borderRadius: "3px" }}>
          {isOwn ? "Add up to 3 all-time favourite shows — hover a show below and click ★ to add it here." : "No shows added to the Encore List yet."}
        </div>
      ) : (
          ...
          {encoreConcerts.map((c, i) => (
            <div key={c.id}>
              <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                <div style={{ color: T.amber, fontFamily: "'Fraunces', serif", fontSize: "28px", fontWeight: "900", minWidth: "32px", textAlign: "center", lineHeight: 1 }}>{i + 1}</div>
                <div style={{ flex: 1 }}><TicketStub concert={c} onOpen={onOpen} isEncore /></div>
                {isOwn && <button onClick={() => onToggleEncore(c.id)} style={{ background: "none", border: "none", color: T.amber, cursor: "pointer", fontSize: "18px", padding: "4px" }}>★</button>}
              </div>
...
const ProfileModal = ({ user, onClose, onToggleFollow }) => {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(26,16,8,.75)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center", padding: "20px", backdropFilter: "blur(4px)" }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ background: T.cream, border: `2px solid ${T.ink}`, borderRadius: "2px", width: "100%", maxWidth: "640px", maxHeight: "90vh", overflowY: "auto", boxShadow: `6px 6px 0 ${T.groove}` }}>
        <div style={{ background: T.ink, padding: "28px 32px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
            <div style={{ display: "flex", gap: "16px", alignItems: "center" }}>
              ...
              <div>
                <div style={{ color: T.amberLight, fontSize: "10px", letterSpacing: "3px", textTransform: "uppercase", fontFamily: "'Outfit', sans-serif", marginBottom: "4px" }}>♪ PUBLIC PROFILE</div>
                <div style={{ color: T.cream, fontFamily: "'Fraunces', serif", fontSize: "22px", fontWeight: "700" }}>{user.name}</div>
            ...
            <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
              <button onClick={() => onToggleFollow(user.id)} style={{ background: user.following ? "transparent" : T.amber, border: user.following ? `1px solid ${T.groove}` : "none", color: user.following ? T.groove : T.ink, borderRadius: "3px", padding: "8px 16px", cursor: "pointer", fontSize: "12px", fontWeight: "700", fontFamily: "'Outfit', sans-serif" }}>
        ...
        <div style={{ padding: "24px 32px" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column" }}>
            {user.concerts.map((c, i) => (<div key={c.id}><TicketStub concert={c} onOpen={setSleeve} />{i < user.concerts.length - 1 && <Groove />}</div>))}
          </div>
        </div>
      </div>
...
const FindUsersModal = ({ onClose, users, onToggleFollow, onViewProfile }) => {
  ...
  const results = q.trim() ? users.filter(u => u.name.toLowerCase().includes(q.toLowerCase()) || u.handle.toLowerCase().includes(q.toLowerCase())) : users;
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(26,16,8,.75)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center", padding: "20px", backdropFilter: "blur(4px)" }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ background: T.cream, border: `2px solid ${T.ink}`, borderRadius: "2px", width: "100%", maxWidth: "460px", boxShadow: `6px 6px 0 ${T.groove}` }}>
        <div style={{ background: T.ink, padding: "24px 28px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
            <div style={{ color: T.amberLight, fontSize: "10px", letterSpacing: "3px", textTransform: "uppercase", fontFamily: "'Outfit', sans-serif" }}>♪ FIND LISTENERS</div>
            <h2 style={{ color: T.cream, fontFamily: "'Fraunces', serif", fontSize: "20px", margin: "4px 0 0" }}>Search the Collection</h2>
          </div>
          <button onClick={onClose} style={{ background: "none", border: "none", color: T.groove, cursor: "pointer" }}><Sym>■</Sym></button>
        </div>
            ...
            {results.map(u => (
              <div key={u.id} style={{ display: "flex", alignItems: "center", gap: "12px", padding: "12px 14px", background: T.paper, border: `1px solid ${T.groove}`, borderRadius: "3px" }}>
                ...
                <div style={{ flex: 1, cursor: "pointer" }} onClick={() => { onViewProfile(u); onClose(); }}>
                  ...
                  <div style={{ color: T.stamp, fontSize: "11px", fontFamily: "'Outfit', sans-serif" }}>{u.handle} · {u.concerts.length} shows · <span style={{ color: T.amber }}><Sym>▶</Sym> View</span></div>
                </div>
                <button onClick={() => onToggleFollow(u.id)} style={{ background: u.following ? "transparent" : T.ink, border: u.following ? `1px solid ${T.groove}` : "none", color: u.following ? T.stamp : T.cream, borderRadius: "3px", padding: "6px 12px", cursor: "pointer", fontSize: "11px", fontWeight: "700", fontFamily: "'Outfit', sans-serif", whiteSpace: "nowrap" }}>
...
const PrivacyPolicy = ({ onClose }) => (
  <div style={{ position: "fixed", inset: 0, background: "rgba(26,16,8,.75)", zIndex: 400, display: "flex", alignItems: "center", justifyContent: "center", padding: "20px", backdropFilter: "blur(4px)" }} onClick={onClose}>
    <div onClick={e => e.stopPropagation()} style={{ background: T.cream, border: `2px solid ${T.ink}`, borderRadius: "2px", width: "100%", maxWidth: "640px", maxHeight: "88vh", overflowY: "auto", boxShadow: `8px 8px 0 ${T.groove}` }}>
      <div style={{ background: T.ink, padding: "24px 32px", display: "flex", justifyContent: "space-between", alignItems: "center", position: "sticky", top: 0, zIndex: 1 }}>
        <div>
          <div style={{ color: T.amberLight, fontSize: "10px", letterSpacing: "3px", textTransform: "uppercase", fontFamily: "'Outfit', sans-serif" }}>♪ Legal</div>
          <h2 style={{ color: T.cream, fontFamily: "'Fraunces', serif", fontSize: "22px", margin: "4px 0 0" }}>Privacy Policy</h2>
        </div>
  ...
  <div style={{ position: "fixed", inset: 0, background: "rgba(26,16,8,.75)", zIndex: 400, display: "flex", alignItems: "center", justifyContent: "center", padding: "20px", backdropFilter: "blur(4px)" }} onClick={onClose}>
    <div onClick={e => e.stopPropagation()} style={{ background: T.cream, border: `2px solid ${T.ink}`, borderRadius: "2px", width: "100%", maxWidth: "640px", maxHeight: "88vh", overflowY: "auto", boxShadow: `8px 8px 0 ${T.groove}` }}>
      <div style={{ background: T.ink, padding: "24px 32px", display: "flex", justifyContent: "space-between", alignItems: "center", position: "sticky", top: 0, zIndex: 1 }}>
        <div>
          <div style={{ color: T.amberLight, fontSize: "10px", letterSpacing: "3px", textTransform: "uppercase", fontFamily: "'Outfit', sans-serif" }}>♪ Legal</div>
          <h2 style={{ color: T.cream, fontFamily: "'Fraunces', serif", fontSize: "22px", margin: "4px 0 0" }}>Terms of Service</h2>
        </div>
        <button onClick={onClose} style={{ background: "none", border: "none", color: T.groove, cursor: "pointer", fontFamily: "'Outfit', sans-serif", fontSize: "13px" }}><Sym>■</Sym> Close</button>
      </div>
      <div style={{ padding: "32px", fontFamily: "'Outfit', sans-serif", color: T.ink, lineHeight: 1.8 }}>
...
const Footer = ({ onPrivacy, onTerms }) => (
  <div style={{ background: T.ink, padding: "28px 32px", borderTop: `4px solid ${T.amber}` }}>
    <div style={{ maxWidth: "760px", margin: "0 auto" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "16px" }}>
        <div>
          <div style={{ color: T.cream, fontFamily: "'Fraunces', serif", fontSize: "20px", fontWeight: "700" }}>{APP_NAME}</div>
      ...
      <div style={{ marginTop: "16px", color: T.stamp, fontSize: "11px", fontFamily: "'Outfit', sans-serif" }}>© {new Date().getFullYear()} {APP_NAME}. All rights reserved. ♪</div>
    </div>
  </div>
);
...
export default function App() {
  ...
  const filtered = concerts
    .filter(c => { const q = search.toLowerCase(); return !q || [c.artist, c.venue, c.city, c.genre].some(x => (x||"").toLowerCase().includes(q)); })
    .sort((a, b) => sort === "date-desc" ? (b.date||"").localeCompare(a.date||"") : sort === "date-asc" ? (a.date||"").localeCompare(b.date||"") : sort === "rating" ? b.rating - a.rating : a.artist.localeCompare(b.artist));

  const feed = users.filter(u => u.following).flatMap(u => u.concerts.map(c => ({ ...c, _owner: u }))).sort((a,b) => (b.date||"").localeCompare(a.date||""));
  ...
  const tabStyle = (t) => ({
    ...
    background: tab === t ? T.amberLight : "transparent",
    color: tab === t ? T.ink : T.groove,
    border: `1.5px solid ${tab === t ? T.amberLight : "rgba(255,255,255,.15)"}`,
    borderRadius: "2px", fontWeight: "700", transition: "all .2s",
  });

  return (
    <div style={{ minHeight: "100vh", background: T.cream, color: T.ink, fontFamily: "'Outfit', sans-serif", display: "flex", flexDirection: "column" }}>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Fraunces:wght@700;900&family=Outfit:wght@400;700&display=swap');
      ...
      <div style={{ background: T.ink, padding: "40px 32px 32px", borderBottom: `4px solid ${T.amber}` }}>
        <div style={{ maxWidth: "760px", margin: "0 auto" }}>
          <div style={{ marginBottom: "22px", display: "flex", flexDirection: "column", gap: "4px" }}>
            {[0.06, 0.1, 0.07].map((op, i) => <div key={i} style={{ height: "1px", background: `rgba(123,200,216,${op})` }} />)}
          </div>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", flexWrap: "wrap", gap: "20px" }}>
            <div>
              <div style={{ color: T.amber, fontSize: "10px", letterSpacing: "5px", textTransform: "uppercase", marginBottom: "10px", fontFamily: "'Outfit', sans-serif" }}>♪ ♫ &nbsp; {APP_SUBTITLE}</div>
              <h1 style={{ fontFamily: "'Fraunces', serif", fontSize: "clamp(52px,8vw,80px)", fontWeight: "900", letterSpacing: "-3px", lineHeight: 0.9, color: "#FAF7F2" }}>{APP_NAME}</h1>
              <p style={{ color: T.groove, fontSize: "14px", marginTop: "12px", fontStyle: "italic", fontFamily: "'Outfit', sans-serif" }}>{APP_TAGLINE}</p>
            </div>
            <button onClick={() => setModal("new")}
              style={{ background: T.amber, color: T.ink, border: "none", borderRadius: "2px", padding: "14px 26px", fontWeight: "700", cursor: "pointer", fontSize: "11px", letterSpacing: "2.5px", textTransform: "uppercase", fontFamily: "'Outfit', sans-serif", boxShadow: `3px 3px 0 ${T.amberLight}`, whiteSpace: "nowrap", transition: "all .15s" }}
              onMouseEnter={e => { e.currentTarget.style.transform = "translate(-1px,-1px)"; e.currentTarget.style.boxShadow = `4px 4px 0 ${T.amberLight}`; }}
              onMouseLeave={e => { e.currentTarget.style.transform = "none"; e.currentTarget.style.boxShadow = `3px 3px 0 ${T.amberLight}`; }}>
              ♪ Press a Record
            </button>
          </div>
          <div style={{ display: "flex", gap: "36px", marginTop: "28px", paddingTop: "20px", borderTop: `1px solid rgba(255,255,255,.08)`, flexWrap: "wrap" }}>
            {[{ l: "Shows Pressed", v: concerts.length }, { l: "Avg Rating", v: avgRating }, { l: "Listeners Following", v: users.filter(u => u.following).length }].map(s => (
              <div key={s.l}>
                <div style={{ color: T.amberLight, fontSize: "24px", fontFamily: "'Fraunces', serif", fontWeight: "700" }}>{s.v}</div>
      ...
      <div style={{ flex: 1, maxWidth: "760px", width: "100%", margin: "0 auto", padding: "28px 32px 40px" }}>
        {tab === "my" && <>
          ...
          {filtered.length === 0
            ? <div style={{ textAlign: "center", padding: "80px 20px" }}>
                ...
                <div style={{ fontSize: "13px", color: T.grooveLight, fontStyle: "italic" }}>{search ? "Try a different search." : "Press your first record to get started."}</div>
              </div>
            : <div style={{ display: "flex", flexDirection: "column" }}>
                {filtered.map((c, i) => (
                  <div key={c.id}>
                    <div style={{ position: "relative" }}>
                      <TicketStub concert={c} onOpen={setSleeve} onEdit={setModal} onDelete={id => setConcerts(cs => cs.filter(x => x.id !== id))} isEncore={encoreList.map(String).includes(String(c.id))} />
                      <button onClick={() => toggleEncore(c.id)}
                        title={encoreList.map(String).includes(String(c.id)) ? "Remove from Encore List" : encoreList.length >= 3 ? "Encore List full (max 3)" : "Add to Encore List"}
                        style={{ position: "absolute", top: 8, right: 52, background: "none", border: "none", cursor: encoreList.length >= 3 && !encoreList.map(String).includes(String(c.id)) ? "not-allowed" : "pointer", fontSize: "16px", color: encoreList.map(String).includes(String(c.id)) ? T.amber : T.groove, opacity: encoreList.length >= 3 && !encoreList.map(String).includes(String(c.id)) ? 0.3 : 1, transition: "color .2s" }}>★</button>
                    </div>
        ...
        {tab === "feed" && <>
          <div style={{ color: T.stamp, fontSize: "12px", fontStyle: "italic", marginBottom: "20px" }}>♪ Recent shows from {users.filter(u => u.following).length} listeners you follow</div>
          {feed.length === 0
            ? <div style={{ textAlign: "center", padding: "80px 20px" }}>
                <div style={{ fontSize: "52px", color: T.groove, marginBottom: "16px" }}>♫</div>
                <div style={{ fontFamily: "'Fraunces', serif", fontSize: "22px", color: T.groove, marginBottom: "8px" }}>The listening room is quiet.</div>
                <div style={{ fontSize: "13px", color: T.grooveLight, fontStyle: "italic" }}>Follow some listeners to hear what they've been to.</div>
              </div>
            : <div style={{ display: "flex", flexDirection: "column" }}>
                {feed.map((c, i) => (<div key={c.id + c._owner.id}><TicketStub concert={c} onOpen={setSleeve} showOwner ownerName={c._owner.name} ownerHandle={c._owner.handle} ownerAvatar={c._owner.avatar} />{i < feed.length - 1 && <Groove />}</div>))}
              </div>
          }
        </>}
        ...
        {tab === "people" && <>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
            ...
            <button onClick={() => setShowFindUsers(true)} style={{ background: T.ink, border: "none", color: T.cream, borderRadius: "2px", padding: "8px 16px", cursor: "pointer", fontSize: "10px", letterSpacing: "2px", textTransform: "uppercase", fontFamily: "'Outfit', sans-serif" }}>♪ Find Listeners</button>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
            {users.map(u => (
              <div key={u.id} style={{ background: T.paper, border: `1.5px solid ${T.groove}`, borderRadius: "3px", padding: "14px 18px", display: "flex", alignItems: "center", gap: "14px", boxShadow: `2px 2px 0 ${T.grooveLight}` }}>
                ...
                <div style={{ flex: 1, cursor: "pointer" }} onClick={() => setProfileView(u)}>
                  ...
                  <div style={{ color: T.stamp, fontSize: "11px" }}>{u.handle} · {u.concerts.length} shows · <span style={{ color: T.amber }}><Sym>▶</Sym> View profile</span></div>
                </div>
                <button onClick={() => toggleFollow(u.id)} style={{ background: u.following ? "transparent" : T.ink, border: u.following ? `1.5px solid ${T.groove}` : "none", color: u.following ? T.stamp : T.cream, borderRadius: "2px", padding: "7px 14px", cursor: "pointer", fontSize: "11px", fontWeight: "700", fontFamily: "'Outfit', sans-serif", letterSpacing: "1px", transition: "all .2s" }}>
                  {u.following ? <><Sym>■</Sym> Unfollow</> : <><Sym>▶</Sym> Follow</>}
                </button>
              </div>
            ))}
